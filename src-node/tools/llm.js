...
Provide a clear, well-structured summary of the day's work.`;
  ...
  {
    name: 'enhance_notes',
    description: 'Enhance technical notes with better clarity and detail',
    parameters: {
      type: 'object',
      properties: {
        note: { type: 'string', description: 'Note text to enhance' },
        context: { type: 'string', description: 'Optional context about the work' },
        style: { 
          type: 'string', 
    ...
    handler: async (args) => {
      try {
        ...
        if (!available) {
          return {
            success: false,
            message: 'LLM not available for note enhancement',
            original: args.note,
            enhanced: args.note,
...
${contextText}Original note: "${args.note}"

Provide an enhanced version of this note.`;

        const result = await ollamaClient.generate(prompt);

        return {
          success: true,
          message: `✅ Note enhanced (${style})`,
          original: args.note,
          enhanced: cleanLlmText(result.response),
          model: result.model,
          style: style
        };
      } catch (error) {
        return {
          success: false,
          message: `Note enhancement failed: ${error.message}`,
          original: args.note,
          enhanced: args.note,
          error: error.message
        };
...
Provide a concise summary of the day's work.`;
  ...
  {
    name: 'llm_enhance_note',
    description: 'Enhance note with technical context and clarity (deprecated: use enhance_notes)',
    parameters: {
      type: 'object',
      properties: {
        note: { type: 'string', description: 'Note text to enhance' },
    ...
    handler: async (args) => {
      try {
        ...
        const style = args.style || 'professional';
        const result = await ollamaClient.enhanceText(args.note, style);

        if (result.success) {
          const enhanced = cleanLlmText(result.enhanced);
          return {
            success: true,
            message: `✅ Note enhanced (${style})`,
            original: args.note,
            ...
            model: result.model,
            style: style,
            charactersSaved: args.note.length - enhanced.length
          };
        } else {
          return {
            success: false,
            message: 'Could not enhance note',
            original: args.note,
            enhanced: args.note,
            error: result.error
          };
        ...
        return {
          ...
          original: args.note,
          enhanced: args.note,
          textResultForLlm: `Note enhancement failed: ${error.message}`,
          resultType: 'failure'
        };
      }
    }
